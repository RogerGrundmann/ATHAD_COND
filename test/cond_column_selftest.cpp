...
static const double X_CO2_CFG  = 0.4109;
static const double X_N2_CFG   = 0.0313;
static const double R_AIR_CFG  = 40.80;       // [kg/m3]
...
int main()
{
    ...
    // The rest of the surface column is CO2 and N2 in the dry ratio.
    const double x_rest      = 1.0 - x_H2O_surf;
    ...
    check("saturated x_CO2 at the surface",           x_CO2_surf, 0.4110, 1e-2);
    check("saturated x_N2  at the surface",           x_N2_surf,  0.0313, 2e-2);
    ...
    check("config x_CO2 matches the saturated value", X_CO2_CFG, x_CO2_surf, 1e-3);
    check("config x_N2  matches the saturated value", X_N2_CFG,  x_N2_surf,  2e-3);
    check_true("config mole fractions sum to 1",
               std::fabs(X_H2O_CFG + X_CO2_CFG + X_N2_CFG - 1.0) < 1e-9);
    ...
    const AtmMixture::Composition S =
        AtmMixture::resolve(X_H2O_CFG, X_CO2_CFG, X_N2_CFG, 0,0,0,0,0);
    check_true("mole fractions sum to 1",             S.valid);
    check("M_mean [g/mol]",     S.M_mean * 1000.0,    29.01,  1e-2);
    check("R_mix [J/(kg K)]",   S.R_mix,              286.6,  1e-2);
    check("q_H2O [kg/kg]",      S.q_H2O,              0.3464, 2e-2);

    const double cp_surf = AtmMixture::cp_of(S.q_H2O, S.q_CO2, T_SURF, S.M_bg);
    check("cp(T_surf) [J/(kg K)]", cp_surf,           1354.0, 3e-2);
    ...
    check("rho_surf [kg/m3]",                         rho_surf, 40.8, 2e-2);
    ...
    check("scale height [m]",   H_surf,               15000.0, 3e-2);

    std::printf("\n mixture properties above the cold trap (the dry composition)\n");
    const AtmMixture::Composition D =
        AtmMixture::resolve(X_H2O_DRY, X_CO2_DRY, X_N2_DRY, 0,0,0,0,0);
    check_true("mole fractions sum to 1",             D.valid);
    check("M_mean [g/mol]",     D.M_mean * 1000.0,    42.63,  1e-3);
    check("R_mix [J/(kg K)]",   D.R_mix,              195.0,  1e-3);
    check("q_H2O [kg/kg]",      D.q_H2O,              0.00423, 2e-2);
    ...
    check("M_nonwater at the surface [g/mol]",        M_nw_surf * 1000.0, 42.888, 1e-2);
